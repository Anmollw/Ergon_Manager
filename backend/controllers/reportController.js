...
const exportTasksReport = async(req,res) => {
    try {
        ...
        res.setHeader(                                             // telling client (new info #3)
            "Content-Type",
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        );

        res.setHeader(
            "Content-Disposition",                                   //handling response(new info#2)
...
const exportUserReport = async(req,res)=>{
    try {
        ...
        const users = await User.find().select("name email _id").lean();           //(new info #3) for devs : used this since the user will go into the excel sheet
